...
　　`YeastNet.v3.benchmark.txt` – 黄金标准基准基因对（用于生成 `geneList.txt`，来源：http://functionalnet.marcottelab.org/yeastnet/）。
...
　　注：数据获取困难：官方需申请 + 专用软件，TAIR 仅存 2014 年旧版，国内多数网站无法访问，建议使用配套预处理文件。
    ...
    AraNet.txt                       - 原始基因相互作用网络（来源：http://functionalnet.marcottelab.org/aranet/download.html）。
    AraNet_GS.txt                    - 金标准基因对网络（来源：http://functionalnet.marcottelab.org/aranet/download.html）。
...
原始数据文件：
    ...
    HumanNet-XC.tsv             - 原始基因相互作用网络（来源：http://functionalnet.marcottelab.org/humannet/download.html）
    HumanNet-GSP.tsv               - 金标准基因对网络（来源：http://functionalnet.marcottelab.org/humannet/download.html）
    go-basic.obo                    - 基因本体术语总库（来源：https://www.geneontology.org/docs/download-ontology/）
...
1. 环境准备
   ...
   - 确保已安装CUDA_Toolkit，本次实验第一部分的矩阵运算需要该工具和cupy库的支持以进行GPU运算。
...
本研究所用的原始数据文件（除 GO 本体库外）均来源于各公共数据库：
   - YeastNet:  http://functionalnet.marcottelab.org/yeastnet/
   - AraNet:    http://functionalnet.marcottelab.org/aranet/download.html
   - HumanNet:  http://functionalnet.marcottelab.org/humannet/download.html
